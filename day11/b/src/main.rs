...
impl Map {
    ...
    fn at(&self, loc: (isize, isize), dir: (isize, isize)) -> Option<State> {
        let x: isize = loc.0 + dir.0;
        let y: isize = loc.1 + dir.1;
        let (w, h) = self.size();
    ...
    fn at_los(&self, loc: (usize, usize), dir: (isize, isize)) -> State {
        let (mut x, mut y) = (loc.0 as isize, loc.1 as isize);
        while let Some(s) = self.at((x, y), dir) {
            match s {
                State::Empty | State::Occupied => return s,
                State::Floor => {
                    x += dir.0;
                    y += dir.1;
                }
    ...
    fn count_neighours(&self, loc: (usize, usize)) -> usize {
        DIRECTIONS
            .iter()
            .filter(|&d| self.at_los(loc, *d) == State::Occupied)
            .count()
    }
